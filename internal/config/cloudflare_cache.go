...
func LoadCloudflareCacheConfigFromEnv() (CloudflareCacheConfig, error) {
	...
	publicBaseURL := strings.TrimSpace(os.Getenv("SONG_SNAPSHOT_PUBLIC_BASE_URL"))
	...
	normalizedBaseURL := ""
	if publicBaseURL == "" {
		validationErrors = append(validationErrors, "SONG_SNAPSHOT_PUBLIC_BASE_URL environment variable is required")
	} else {
		parsedURL, err := url.ParseRequestURI(publicBaseURL)
		if err != nil {
			validationErrors = append(validationErrors, "SONG_SNAPSHOT_PUBLIC_BASE_URL must be a valid HTTPS URL")
		} else {
			switch {
			case parsedURL.Scheme != "https":
				validationErrors = append(validationErrors, "SONG_SNAPSHOT_PUBLIC_BASE_URL must use HTTPS")
			case parsedURL.Host == "":
				validationErrors = append(validationErrors, "SONG_SNAPSHOT_PUBLIC_BASE_URL must include a host")
			case parsedURL.User != nil || (parsedURL.Path != "" && parsedURL.Path != "/") || parsedURL.RawQuery != "" || parsedURL.Fragment != "":
				validationErrors = append(validationErrors, "SONG_SNAPSHOT_PUBLIC_BASE_URL must not include credentials, a path, query, or fragment")
			default:
	...
	allowedDestinations := map[string]string{
		info.DevelopSongSnapshotPublicBaseURL:    info.DevelopCloudflareZoneID,
		info.BetaSongSnapshotPublicBaseURL:       info.BetaCloudflareZoneID,
		info.ProductionSongSnapshotPublicBaseURL: info.ProductionCloudflareZoneID,
	}
	if expectedZoneID, ok := allowedDestinations[normalizedBaseURL]; !ok || expectedZoneID != zoneID {
		return CloudflareCacheConfig{}, fmt.Errorf("cloudflare cache configuration validation failed: CLOUDFLARE_ZONE_ID and SONG_SNAPSHOT_PUBLIC_BASE_URL must match a configured deployment destination")
	}
