...
func (s *userCredentialUsecaseImpl) validateReauthenticatedUser(user *entity.User, reauthInfo *RecentSignInInfo, operation string) (string, error) {
	...
	if firebaseUID == "" {
		slog.Warn(
			reauthenticationFailureMessage(operation),
			"reason",
...
func reauthenticationFailureMessage(operation string) string {
	if operation == "delete_account" {
		return "suspicious account deletion authentication failure"
	}
	return "suspicious " + operation + " authentication failure"
}
