...
func TestApplyScores_通常譜面とWORLDSENDを保存し通常譜面だけをOVERPOWER集計する(t *testing.T) {
	...
	// When
	counts, skipped, changes, statistics, overpower, err := uc.applyScores(context.Background(), nil, 99, payload, masters, updatedAt)
	...
	assert.Equal(t, api_internal.PlayerDataStatistics{
		TotalHighScore: 1010000,
		LampCounts: api_internal.PlayerDataLampCounts{
			Clear:     map[string]int{"ABSOLUTE": 1},
			Combo:     map[string]int{"ALL JUSTICE": 1},
			FullChain: map[string]int{"FULL CHAIN GOLD": 1},
		},
	}, statistics)
...
func TestApplyScores_既存レコードを含めてOVERPOWERを再計算する(t *testing.T) {
	...
	// When
	_, _, _, _, overpower, err := uc.applyScores(context.Background(), nil, 99, payload, masters, updatedAt)
...
func TestApplyScores_未解禁曲を除外してOVERPOWERを再計算する(t *testing.T) {
	...
	// When
	_, _, _, _, overpower, err := uc.applyScores(context.Background(), nil, 99, PlayerDataScorePayload{}, newApplyScoresTestMasters(), updatedAt)
...
func TestApplyScores_既存レコード取得失敗時はエラーを返す(t *testing.T) {
	...
	// When
	_, _, _, _, _, err := uc.applyScores(context.Background(), nil, 99, PlayerDataScorePayload{}, newApplyScoresTestMasters(), time.Date(2026, 4, 27, 0, 0, 0, 0, time.UTC))
...
func TestApplyScores_既存レコードの関連情報欠損時はスキップして集計する(t *testing.T) {
	...
	// When
	_, _, _, _, overpower, err := uc.applyScores(context.Background(), nil, 99, PlayerDataScorePayload{}, newApplyScoresTestMasters(), time.Date(2026, 4, 27, 0, 0, 0, 0, time.UTC))
...
func TestApplyScores_不正レコードをスキップして理由を保持する(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// When
			counts, skipped, changes, _, overpower, err := uc.applyScores(context.Background(), nil, 77, tt.payload, newApplyScoresTestMasters(), time.Date(2026, 4, 27, 0, 0, 0, 0, time.UTC))
...
func TestApplyScores_保存前状態との差分を返す(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantChanges: []api_internal.PlayerDataRecordChange{
				{
					RecordType: "full", ChangeType: "new", Idx: "full-song", Diff: "MASTER",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// When
			counts, skipped, changes, _, _, err := uc.applyScores(context.Background(), nil, 77, tt.payload, newApplyScoresTestMasters(), time.Date(2026, 4, 27, 0, 0, 0, 0, time.UTC))
...
func TestBuildPlayerDataStatistics_TOTALHIGHSCOREとランプ件数を集計する(t *testing.T) {
	// Given
	records := []*entity.PlayerRecord{
		{
			Score:     1000000,
			ClearLamp: &entity.ClearLampType{Name: "CLEAR"},
			ComboLamp: &entity.ComboLampType{Name: "none"},
			FullChain: &entity.FullChainType{Name: "none"},
		},
		{
			Score:     1010000,
			ClearLamp: &entity.ClearLampType{Name: "ABSOLUTE"},
		...
		},
		{
			Score:     990000,
			ClearLamp: &entity.ClearLampType{Name: "CLEAR"},
			ComboLamp: &entity.ComboLampType{Name: "FULL COMBO"},
			FullChain: &entity.FullChainType{Name: "none"},
		},
		nil,
	}
	...
	// When
	statistics := buildPlayerDataStatistics(records)

	// Then
	assert.Equal(t, api_internal.PlayerDataStatistics{
		TotalHighScore: 3000000,
		LampCounts: api_internal.PlayerDataLampCounts{
...
func TestSortAndLimitRecordChanges_idx数値順で並べ最大100件に制限する(t *testing.T) {
	...
	changes = append(changes,
		...
		api_internal.PlayerDataRecordChange{RecordType: "full", ChangeType: "new", Idx: "not-number", Diff: "MASTER"},
	)
