...
func TestIsKnownAccountType(t *testing.T) {
	tests := []struct {
		...
		{name: "EXTDEVは既知ロール", accountTypeID: AccountTypeExtDev, want: true},
		{name: "0は未知ロール", accountTypeID: 0, want: false},
		{name: "5は未知ロール", accountTypeID: 5, want: false},
...
func TestHasRole(t *testing.T) {
	tests := []struct {
		...
		{name: "PLAYER要求にEXTDEVは成功", accountTypeID: AccountTypeExtDev, requiredRoleID: AccountTypePlayer, want: true},
		{name: "EDITOR要求にEXTDEVは失敗", accountTypeID: AccountTypeExtDev, requiredRoleID: AccountTypeEditor, want: false},
		{name: "ADMIN要求にEXTDEVは失敗", accountTypeID: AccountTypeExtDev, requiredRoleID: AccountTypeAdmin, want: false},
		{name: "EXTDEV要求にEXTDEVは成功", accountTypeID: AccountTypeExtDev, requiredRoleID: AccountTypeExtDev, want: true},
		{name: "EXTDEV要求にPLAYERは失敗", accountTypeID: AccountTypePlayer, requiredRoleID: AccountTypeExtDev, want: false},
		{name: "未知ロールIDは失敗", accountTypeID: 5, requiredRoleID: AccountTypeAdmin, want: false},
		{name: "未知ロールIDは数値が大きくても失敗", accountTypeID: 999, requiredRoleID: AccountTypeAdmin, want: false},
		{name: "未知のrequiredRoleIDは失敗", accountTypeID: AccountTypeAdmin, requiredRoleID: 5, want: false},
	}
