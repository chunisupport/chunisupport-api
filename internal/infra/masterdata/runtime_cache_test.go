...
func TestRuntimeCache_初期ロードに失敗した場合はエラーを返す(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "ロード処理がnilキャッシュを返した場合",
			loader: &masterLoaderMock{dynamic: nil, static: &StaticCache{}},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			runtime, err := NewRuntimeCache(context.Background(), tt.loader)
