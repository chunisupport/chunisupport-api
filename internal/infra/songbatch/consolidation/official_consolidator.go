...
func (c *OfficialConsolidator) prepareSongsForUpsert() ([]*models.SongModelForUpsert, map[string]struct{}) {
	...
	for _, officialSong := range *c.data {
		...
		songModel.IsNew = utils.BoolToInt(strings.TrimSpace(officialSong.Newflag) == "1")
...
func (c *OfficialConsolidator) detectMassiveIdxChange(ctx context.Context, existingActiveSongs map[string]int) error {
	...
	for _, song := range *c.data {
		...
		isWEInt := utils.BoolToInt(isWorldsEnd)
...
func loadDifficultyMap(ctx context.Context, repo songbatch.DifficultyRepository) (map[string]int, error) {
...
func loadGenreMap(ctx context.Context, repo songbatch.GenreRepository) (map[string]int, error) {
