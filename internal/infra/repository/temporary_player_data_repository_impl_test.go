...
func TestTemporaryPlayerDataRepository_CreateAndFind(t *testing.T) {
	...
	got, err := repo.FindByToken(context.Background(), "token")
	require.NoError(t, err)
...
func TestTemporaryPlayerDataRepository_IP上限(t *testing.T) {
	...
	err := repo.Create(context.Background(), &entity.TemporaryPlayerData{Token: "t1", IPAddress: "127.0.0.1", Payload: []byte("{}"), ExpiresAt: time.Now().UTC().Add(time.Minute)})
	require.NoError(t, err)

	err = repo.Create(context.Background(), &entity.TemporaryPlayerData{Token: "t2", IPAddress: "127.0.0.1", Payload: []byte("{}"), ExpiresAt: time.Now().UTC().Add(time.Minute)})
	require.Error(t, err)
...
func TestTemporaryPlayerDataRepository_総量上限(t *testing.T) {
	...
	err := repo.Create(context.Background(), &entity.TemporaryPlayerData{Token: "t1", IPAddress: "127.0.0.1", Payload: []byte("1234"), ExpiresAt: time.Now().UTC().Add(time.Minute)})
	require.NoError(t, err)

	err = repo.Create(context.Background(), &entity.TemporaryPlayerData{Token: "t2", IPAddress: "127.0.0.2", Payload: []byte("1"), ExpiresAt: time.Now().UTC().Add(time.Minute)})
	require.Error(t, err)
...
func TestTemporaryPlayerDataRepository_期限切れは参照不可(t *testing.T) {
	repo := NewTemporaryPlayerDataRepository(3, 1024)
	err := repo.Create(context.Background(), &entity.TemporaryPlayerData{Token: "t1", IPAddress: "127.0.0.1", Payload: []byte("{}"), ExpiresAt: time.Now().UTC().Add(-time.Second)})
	require.NoError(t, err)

	_, err = repo.FindByToken(context.Background(), "t1")
	require.Error(t, err)
