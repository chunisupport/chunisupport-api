	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err = db.Exec(`
				...
				VALUES (1, 'c1', 'Song 1', 'Artist 1', 1, 180, 'idx-1', 0, 0, 0)
			`)
			require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := db.Exec(`
				INSERT INTO songs (id, display_id, title, artist, genre_id, bpm, official_idx, is_worldsend, is_new, is_deleted)
				VALUES
					(1, 'c1', 'Song 1', 'Artist 1', 1, 180, 'idx-1', 0, 0, 0),
					(2, 'c2', 'Song 2', 'Artist 2', 1, 180, 'idx-2', 0, 0, 0),
					(3, 'c3', 'Worldsend Song', 'Artist 3', 1, 180, 'idx-3', 1, 0, 0),
					(4, 'c4', 'Deleted Song', 'Artist 4', 1, 180, 'idx-4', 0, 0, 1)
			`)
			require.NoError(t, err)
