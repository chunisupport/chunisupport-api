...
func (r *courseRepository) Create(ctx context.Context, exec domainrepo.Executor, course *entity.Course) error {
	...
	result, err := exec.ExecContext(ctx, `INSERT INTO courses (official_idx, name, course_class_id, is_deleted) VALUES (?, ?, ?, ?)`, course.OfficialIdx, course.Name, course.CourseClassID, course.IsDeleted)
	if err != nil {
