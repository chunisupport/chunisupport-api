...
// HasOfficialMetricsChanged は、現在の公式RATING・公式OVER POWERの組から変化するかを判定します。
func (p *Player) HasOfficialMetricsChanged(rating, overpower float64) bool {
	return p.OfficialRating != rating || p.OfficialOverpower != overpower
}

// ChangeOfficialMetrics は取得日時の単調性を守りつつ公式指標を更新し、必要なら更新前状態を履歴化します。
func (p *Player) ChangeOfficialMetrics(rating, overpower float64, collectedAt time.Time) error {
	if p.DataCollectedAt != nil {
		if collectedAt.Before(*p.DataCollectedAt) {
			return ErrStalePlayerData
		}
		if collectedAt.Equal(*p.DataCollectedAt) && p.HasOfficialMetricsChanged(rating, overpower) {
			return ErrConflictingPlayerDataTimestamp
		}
		if p.HasOfficialMetricsChanged(rating, overpower) {
			p.metricHistoryToAppend = &PlayerMetricHistoryEntry{
				PlayerID: p.ID, OfficialRating: p.OfficialRating,
				OfficialOverpower: p.OfficialOverpower, DataCollectedAt: *p.DataCollectedAt,
			}
		}
	}
