...
type WorldsendChartRepository interface {
	...
	FindAll(ctx context.Context, includeDeleted bool) ([]*WorldsendSongWithChart, error)

	// FindByDisplayID は指定された DisplayID の WORLD'S END 楽曲を取得します。
	// 削除済み楽曲も取得します。
	FindByDisplayID(ctx context.Context, displayID string) (*WorldsendSongWithChart, error)
	...
	UpdateSongs(ctx context.Context, songs []*entity.Song, charts []*entity.WorldsendChart) error
}
