...
func TestGetWorldsendSongs(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/internal/songs/worldsend", nil)
	...
	var response api_internal.WorldsendSongsResponse
	...
	if len(response.Songs) != 1 {
		t.Fatalf("Songs count = %d, want 1", len(response.Songs))
	}
}
