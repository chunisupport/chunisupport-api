...
外部利用者向けの楽曲・譜面一覧JSONをCloudflare R2へ保存するワンショットバッチです。
...
| 環境変数 | 内容 | 例 |
| --- | --- | --- |
| `R2_ENDPOINT_URL` | R2のS3互換APIエンドポイント。HTTPSのオリジンのみ指定 | `https://<ACCOUNT_ID>.r2.cloudflarestorage.com` |
| `R2_ACCESS_KEY_ID` | 対象バケットへ書き込み可能なR2 APIトークンのAccess Key ID | `...` |
| `R2_SECRET_ACCESS_KEY` | 同じR2 APIトークンのSecret Access Key | `...` |
| `R2_BUCKET_NAME` | 公開用オブジェクトを保存する既存バケット名 | `chunisupport-public` |
...
両方のJSONはR2への書き込み開始前に生成します。ただし、2オブジェクトのPUTは単一トランザクションではありません。
...
- 実行間隔: 6時間
- 正常終了: 終了コード `0`
- 設定、DB取得、JSON生成、R2アップロードの失敗: 終了コード `1`
