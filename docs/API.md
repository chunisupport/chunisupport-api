...
1. **本枠の判定**:
   ...
   - 日次バッチでは現行版プレイヤーの正常な公式本枠を保持します
   - 旧版プレイヤーと、`best` / `new` 本枠の件数・`slot_order` が不正な現行版プレイヤーは、最新マスタと対象となる通常譜面のスコアから枠を再構築します
...
- **制限**: 履歴はプレイヤーごとに最大500件で、レスポンス先頭の現行値を含めると最大501件です。
- **パスパラメータ**: `username` - 対象ユーザーのユーザー名
- **レスポンス**: 200 OK
