...
- 1プレイヤーあたり履歴テーブルは最大500件。超過分は `data_collected_at` が古い行から削除する。現在値は `Player` が保持し、この件数に含めない
...
- レーティング帯は 0.1刻みで定義（15.00未満、15.00～17.60、17.70+）
...
移行ファイルにはサーバー固有IDを含めません。楽曲とコースはofficial_idx、譜面はofficial_idxと大文字の難易度名、各種マスターは名称で参照します。目標属性のdiff、genre、verも名称へ外部化し、移行先で一括解決します。称号タイプは移行先マスターから名称で解決し、称号本体が存在しない場合は移行データの名称、称号タイプ、画像URLから追加します。
