...
### ChartStatistics（譜面統計エンティティ）

#### 概要
譜面ごと・レーティング帯ごとのランク・ランプ人数を集計した統計情報を表すエンティティ。
バッチ処理により定期的に更新され、API経由で提供されます。

#### フィールド

| フィールド名 | 型 | 必須 | 説明 |
|------------|-----|-----|------|
| ChartID | int | ✓ | 譜面ID（外部キー、複合主キーの一部） |
| RatingTier | int | ✓ | レーティング帯（複合主キーの一部）<br>150-176: 15.0-17.6（0.1刻み）<br>177: 17.7以上 |
| RankS | int | ✓ | Sランク人数 (975,000-989,999) |
| RankSPlus | int | ✓ | S+ランク人数 (990,000-999,999) |
| RankSS | int | ✓ | SSランク人数 (1,000,000-1,004,999) |
| RankSSPlus | int | ✓ | SS+ランク人数 (1,005,000-1,007,499) |
| RankSSS | int | ✓ | SSSランク人数 (1,007,500-1,008,999) |
| RankSSSPlus | int | ✓ | SSS+ランク人数 (1,009,000+) |
...
- **IsValidRatingTier() bool**
  - レーティング帯が有効な値（150-177）かを判定
  - 返り値: `RatingTier >= 150 && RatingTier <= 177`

- **GetRatingTierString() string**
  - レーティング帯の文字列表現を返す
  - 返り値例: `"15.0"`, `"16.5"`, `"17.7+"`

#### 不変条件

- `RatingTier` は150～177の範囲
- `ChartID` + `RatingTier` の組み合わせは一意
- 集計対象: 譜面定数10.0以上の譜面 × ベスト枠平均レーティング15.0以上のプレイヤー
- バッチ処理による更新のため、リアルタイムデータではない

#### 設計意図

- N+1問題を回避するため、事前集計データとして管理
- レーティング帯を複合主キーに含めることで、1譜面あたり最大28レコード（15.0-17.6: 27区分 + 17.7+: 1区分）を保持
- 譜面定数10.0未満の譜面は統計データを持たない（API応答では `statistics: null`）
