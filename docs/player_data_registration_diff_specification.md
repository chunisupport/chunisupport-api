...
`scores.standard` は公式レーティング枠の完全な写しとして扱う。枠の件数・順位範囲・順位重複を正規化後の保存対象で検証し、成功した場合は同一トランザクション内で既存の通常譜面枠をすべて `none` にしてからpayload由来の枠を保存する。payloadにない譜面や解決時にスキップされた譜面ではスコア行を残し、枠だけを解除する。空配列でも既存枠をすべて解除する。枠の解除は `changes` と実変更件数に含めず、枠だけが変わる既存行の `updated_at` も更新しない。
