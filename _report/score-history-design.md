...
3. Repository が現行レコードと histories を **単一 SQL** で取得する
   - `UNION ALL` で現行レコードと histories を結合し、現行レコードを先頭、histories を `updated_at DESC` で並べる
   - 現行レコードには内部的な並び替え用フラグを付けるが、API レスポンスには露出しない
   - 現行レコードと histories を別々の SQL で取得してはならない。取得間の更新コミットによる重複・欠落を防ぎ、1 ステートメントの一貫したスナップショットを保証する
   - 現行レコードがなければ `404`（`score_history_not_found`）。histories のみ存在する不整合時もフォールバックせず同じく `404`
4. Repository の取得結果をレスポンス DTO へ変換する
...
同一譜面を同一秒に複数回更新する登録操作はサポートしない。この制約は、履歴の主キー `(player_id, chart_id, updated_at)` により保証する。

衝突した場合は仕様どおり登録失敗とし、既存履歴を上書き・無視せず、履歴 INSERT の主キー重複エラーによって登録トランザクション全体をロールバックする。Repository はこの重複を `ErrScoreHistoryTimestampConflict` へ変換し、呼び出し元は登録失敗として明示的にエラーを返す。
...
4. 現行レコード + histories を Repository の単一 SQL で取得して DTO 化
...
type PlayerRecordHistoryRepository interface {
    FindStandardStatesForUpdate(ctx context.Context, playerID int, chartIDs []int) (map[int]PlayerRecordState, error)
    FindWorldsendStatesForUpdate(ctx context.Context, playerID int, chartIDs []int) (map[int]WorldsendRecordState, error)
    BulkInsertStandard(ctx context.Context, rows []PlayerRecordHistoryRow) error
    BulkInsertWorldsend(ctx context.Context, rows []WorldsendRecordHistoryRow) error
    PruneStandardOverLimit(ctx context.Context, playerID int, chartIDs []int) error
    PruneWorldsendOverLimit(ctx context.Context, playerID int, chartIDs []int) error
    FindStandardTimeline(ctx context.Context, playerID, chartID int) ([]PlayerRecordHistoryRow, error)
    FindWorldsendTimeline(ctx context.Context, playerID, worldsendChartID int) ([]WorldsendRecordHistoryRow, error)
}
```

- Repository インターフェースには `Executor`、SQL、`sqlx` などの永続化実装詳細を露出させない
- 書き込みでは、Usecase 層に定義するスコア登録用 Unit of Work がトランザクション境界を管理し、コールバックへ同一トランザクションに束縛済みの Repository 群を渡す
- Usecase は受け取った Repository のメソッドだけを呼び出し、トランザクションオブジェクトを直接操作しない
- 読み取りの `FindStandardTimeline` / `FindWorldsendTimeline` は、現行レコードと histories を `UNION ALL` した単一 SQL で返す。現行レコードの有無も同じ結果から判定する
...
スコア登録用 Unit of Work の契約は Usecase 層に定義する。

```go
// 概略（実装時に既存の登録処理で必要な Repository を確定する）
type PlayerDataRepositories struct {
    PlayerData    repository.PlayerDataRepository
    ScoreHistory repository.PlayerRecordHistoryRepository
}

type PlayerDataUnitOfWork interface {
    Transactional(ctx context.Context, fn func(repos PlayerDataRepositories) error) error
}
```

Infra 層はトランザクション開始後、そのトランザクションに束縛した Repository 実装を `PlayerDataRepositories` として生成する。トランザクションの commit / rollback は Unit of Work 実装だけが担当する。
...
- 1 リクエストあたり DB アクセス目標: **3 回以内**
  1. ユーザー解決
  2. 譜面解決（マスタキャッシュで解決できる場合は DB アクセスなし）
  3. 現行レコード 1 件 + histories 最大 50 件を単一 SQL で取得
- 複数譜面の一括取得 API は設けない
...
9. テスト
   ...
   - BAS/ADV API 要求で 400
   - 未プレイで 404
   - 履歴取得と同時に更新がコミットされても、現行値と histories に重複・欠落がない
...
| 履歴取得と同時にスコア更新がコミット | 単一 SQL のスナップショットにより、現行値と histories の重複・欠落がない |
