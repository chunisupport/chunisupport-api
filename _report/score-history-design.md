...
- 同一譜面の同時登録リクエストは通常発生しない（既存 `applyScores` と同じ前提）。この前提が崩れて histories が一時的に 51 行を超えても許容し、次回 prune で解消する（§4.4）
...
1. 既存どおり `before` 状態をキャプチャ（`FindPlayerRecordStatesByChartIDs` 等）
...
#### 移行時の注意（既存テーブルへのパーティション追加）

`ALTER TABLE ... PARTITION BY HASH(player_id)` はテーブル全体の再構築を伴う。初回リリース前であればデータ量は少なく影響は限定的だが、本番データが既に蓄積されている場合は以下を考慮する。

| 観点 | 内容 |
| --- | --- |
| 所要時間 | 行数・行サイズ・Secondary Index に比例。目安として 1,000 万行で数分〜数十分（環境依存） |
| 推奨手順 | メンテナンスウィンドウを確保し、`ALTER` 前にバックアップを取得 |
| 代替案 | ダウンタイムを許容できるなら、新テーブルへ `CREATE ... PARTITION` → データコピー → リネーム（dump & restore 相当）の方が計画しやすい場合がある |
| ロールバック | `down` マイグレーションで `REMOVE PARTITIONING`（これも再構築を伴う） |
...
- 51 件目 INSERT 後、同一トランザクション内で超過分を DELETE（同期 prune）
- 50 件を超えた古い履歴は永久に失われる（仕様）

#### 同時登録時の競合

2 トランザクションが同一譜面に同時 INSERT → prune した場合、最古行 DELETE が競合し一時的に 51 行超過しうる（§2.3 の前提が崩れた場合）。**51 行超過を許容し、次回の登録時 prune で 50 行以内に収束させる**。厳密な同期は求めない。
...
1. 既存どおり `before` 状態を取得し、`computeFullRecordChanges` / `computeWorldsendRecordChanges` で差分算出
2. `updated` かつ対象難易度のレコードについて、histories への INSERT 対象を収集
3. histories へバルク INSERT（`before` 状態）
4. `SavePlayerData` で現行レコードを UPSERT
5. 今回 INSERT した譜面 ID に対して prune（同一トランザクション）
...
件数上限は Repository 実装が `info.MaxScoreHistoryEntriesPerChart` を参照する。インターフェースに `limit` 引数は持たせない。
...
1. マイグレーション（**2 ファイルに分割**。既存最新は `000020` のため、次は `000021` / `000022` を想定）
   - `000021`: `player_records` / `player_worldsend_records` へのパーティション適用
   - `000022`: `player_record_histories` / `player_worldsend_record_histories` 作成（作成時からパーティション付き）
2. Domain: 履歴行・難易度判定
...
| prune の同時実行競合 | 51 行超過を許容。次回 prune で 50 行以内に収束 |
