...
1. 対象となる現行レコードを `SELECT ... FOR UPDATE` でロックする
2. 対象難易度（後述）を満たす場合、**更新前状態（before）** を histories へ INSERT
   - `updated_at` には `before.UpdatedAt`（そのスコアを登録した日時）を保存
3. `player_records` を after で UPDATE
4. 対象譜面について histories の 51 件目以降を同期 DELETE（prune）
...
4. histories INSERT、`SavePlayerData`、prune の順に実行（同一トランザクション）
...
| private 本人以外 | 404 |
| private 本人 | 200 |
| 楽曲は存在するが指定難易度の譜面が存在しない | 既存の楽曲 API に準拠した 404 |
