# リファクタリング指摘書 (2026-07-10時点 / セキュリティ追記 2026-07-22)

本ドキュメントは、現行コードベースを再確認したうえで、**未解決の改善点のみ**を整理したものです。
解消済みの項目、現行実装では成立しない項目、他項目と重複する指摘は削除または統合しています。
2026-07-22 のセキュリティレビューで、認可・トークン・レート制限・依存関係の未記載項目を SEC 節へ追記しました。
...
| **SEC-09** | **Medium** | フレンド申請が非公開ユーザー名の存在オラクルになる | プロフィール系は非公開を `ErrUserPrivate` → 404 に正規化して存在を隠蔽しますが、`internal/usecase/friendship_usecase_impl.go` の `SendRequest` は非公開ユーザーにも申請でき、存在すれば 204 や競合などで成否が分かれます。ユーザー名の推測・総当りで非公開アカウントの有無を確認できます。SEC-08 の詳細開示とは別問題です。未フレンドの非公開相手への申請を拒否し、応答・タイミングを「存在しない」と区別できない形に揃えるか、申請可否自体を公開設定と整合させるべきです。 |
| **SEC-10** | **Medium** | APIトークンに有効期限・スコープ・利用監査がない | `internal/usecase/api_token_usecase_impl.go` は 32 バイト乱数と SHA-256 保存で発行し、平文は保存しません。一方でトークンは無期限・ユーザーあたり1件・スコープなしで、外部API（`/v1`・互換API）上の当該ユーザー権限をすべて代行します。`last_used_at` やローテーション履歴もなく、漏洩時の被害期間と影響範囲を限定できません。有効期限、読み取り専用スコープ、最終利用時刻、失効・再発行の運用を導入すべきです。 |
| **SEC-11** | **Medium** | レート制限がプロセス内メモリのみで水平展開に弱い | `internal/app/middleware/rate_limit_middleware.go` の `FixedWindowStore` は単一プロセスの map です。複数インスタンスでは制限がインスタンス数分だけ緩み、再起動でカウンタが消えます。Fixed Window のため境界付近のバーストも起きやすいです。ログイン・登録・一時データ・公開APIの耐性が本番構成次第で弱くなります。Redis 等の共有ストアへ移すか、リバースプロキシ側制限を必須として文書化すべきです。`HDL-001` の IP 抽出誤りと組み合わさると効果が大きく落ちます。 |
| **SEC-12** | **Medium** | 認証済み内部の状態変更APIにユーザー単位レート制限がない | `internal/app/router.go` で User/IP レート制限があるのは login/signup、register-data、temp commit、公開GET、外部APIトークン経路に限られます。`/internal/friends`、`/internal/auth/api-tokens`、goals・favorite・locked-songs などの状態変更には `UserRateLimitMiddleware` がありません。送信申請の上限100は保存件数の上限であり試行回数を抑えません。`PERF-008` の受信無上限と組み合わさると申請スパムやトークン再発行連打が容易です。少なくともフレンド申請とAPIトークン発行へユーザー単位制限を付与すべきです。 |
| **SEC-13** | **Low** | 一時プレイヤーデータの uploadToken が Commit 時に主体へ紐づかない | `internal/usecase/temporary_player_data_usecase_impl.go` の `Commit` はトークン所持と認証済みユーザーのみを条件とし、アップロード時 IP や主体との一致を検証しません。gzip/展開サイズ上限と TTL 5分・IPあたり件数上限はあります。トークン漏洩時は被害者アカウントの改ざんではなく、他人のゲームデータが攻撃者アカウントへ取り込まれる形の情報漏えいになります。TTL とワンタイム消費を維持しつつ、ログへのトークン出力禁止、必要なら緩い IP 一致や用途限定を検討すべきです。 |
| **SEC-14** | **Low** | 読み取り最適化認証が失効確認を省略する | `internal/usecase/read_optimized_token_verifier.go` と `optionalFirebaseAuthReadOptimized` は、楽曲・コース等の公開GETおよび `/internal/users/:username/updated-at` で Firebase の失効確認なし検証を使います。書き込みとユーザープロフィール系は strict 側で分離済みです。失効直後でも有効期限内 JWT なら、非公開フレンドの `updated-at` 等へアクセスできる可能性があります。影響は限定的ですが、許容する経路と最大遅延を運用ドキュメントに明示すべきです。 |
| **SEC-15** | **Low** | 管理者によるユーザー削除に recent sign-in がない | 本人退会（`DeleteOwnAccount`）は `X-Reauth-Token` と `auth_time` 5分以内を要求しますが、管理者削除（`DeleteUser`）は Firebase 認証と ADMIN ロールのみです。管理者セッションが乗っ取られた場合の被害が大きい操作のため、再認証または追加の監査（誰が誰を削除したか）を検討すべきです。 |
| **SEC-16** | **Low** | Go 標準ライブラリ GO-2026-5856（crypto/tls ECH プライバシー） | `govulncheck` で `crypto/tls` の GO-2026-5856 / CVE-2026-42505 が go1.26.4 に該当します（修正は go1.26.5）。Encrypted Client Hello 利用時に PSK 識別子が漏れる問題です。本APIは `127.0.0.1` 待受が主でサーバ側影響は限定的ですが、Turnstile・Firebase などアウトバウンド TLS に波及し得ます。ランタイムを Go 1.26.5 以上へ更新すべきです。 |
...
- 2026-07-22 のセキュリティレビューでは Critical な認証バイパスや SQL インジェクションは見つかりませんでした。一方で **非公開ユーザー名の存在オラクル (`SEC-09`)**、**APIトークンの無期限・無スコープ (`SEC-10`)**、**プロセスローカルなレート制限 (`SEC-11`)**、**認証済み状態変更のレート制限不足 (`SEC-12`)** を新規に SEC へ追加しました。`SEC-09` / `SEC-12` は `SEC-08` および `PERF-008` とあわせて対応するのが効率的です。
