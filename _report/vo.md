...
- `Scan` 実装は全 VO で統一し、「DB からの復元 = `NewXxxFromRepository` を経由して生成」を原則とする
- これにより「信頼済みデータの復元時は再バリデーションしない / 外部入力は `NewXxx` で必ずバリデーションする」をコードで強制する
- 可能な限り振る舞い（例: `IsRankedTarget`）を型に持たせる
- 既存 VO（`username`, `playername`, `score`, `chartconstant`, `notes`）は実装差分を棚卸しし、
  段階的に以下へリファクタリングする
  - 生成経路の二系統化（`NewXxx` / `NewXxxFromRepository`）
  - `Scan` の責務統一（復元は `NewXxxFromRepository` のみを利用）
  - バリデーション場所の統一（外部入力境界に寄せる）
  - 互換性維持のため、既存 API/DB 仕様は変更せず内部実装のみ置換する
...
3. 変更対象の合意（第1群から着手）
4. 既存 VO の `Scan` / コンストラクタ実装を調査し、統一ガイドラインとの差分を記録する

完了条件:

- 影響範囲一覧が揃っている
- 現行テストが安定してグリーン
- 既存 VO のリファクタリング対象（優先度つき）が整理されている
