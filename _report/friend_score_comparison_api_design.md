...
主な利用形態は `MASTER` と `ULTIMA` の比較です。画面はそれぞれの難易度を個別に取得し、それ以外の難易度への切り替え時にも必要な難易度だけを取得します。
...
未プレイのためだけに疑似レコードをDBへ保存してはいけません。RepositoryはLEFT JOIN結果に対応するレコードの有無をポインタで表現し、Usecaseが上記のAPI出力値へ正規化します。DTO変換では比較ルールや未プレイ補完を行いません。

`is_played` はスコア値ではなく、`player_records` 行の存在で判定します。したがって、スコア0のレコードが存在する場合は `is_played: true` です。
...
| 自分 | フレンド | 結果 |
| --- | --- | --- |
| プレイ済み・スコアが0より大きい | 未プレイ | `SELF_WIN` |
| 未プレイ | プレイ済み・スコアが0より大きい | `FRIEND_WIN` |
| プレイ済み・スコア0 | 未プレイ | `DRAW` |
| 未プレイ | プレイ済み・スコア0 | `DRAW` |
| 未プレイ | 未プレイ | `DRAW` |
...
全件を返すことで、フロントエンドは追加リクエストなしで集計値と一覧を一致させられます。全件返却の可否は、実データ相当の最大件数でのレスポンスサイズと処理負荷を実装時に確認して判断します。許容値を満たさない場合は、本APIを公開する前にカーソルページングとサーバー側集計を含む仕様へ改めます。
...
比較UsecaseによるDBアクセスは、1リクエストあたり原則2クエリとします。Firebase認証でのユーザー取得など、共通ミドルウェアのDBアクセスは含めません。
...
主な利用形態である `MASTER` と `ULTIMA` をそれぞれ取得する場合を、既存の全レコードAPIで自分とフレンドの2人分を取得する場合と比較します。両難易度の比較UsecaseによるDBアクセスは合計4クエリです。両難易度の合計転送量と処理負荷を重視し、それ以外の難易度への切り替え時間は主な最適化対象にしません。具体的な数値は実環境計測後に確定しますが、受け入れ基準として少なくとも次を満たすものとします。

- 比較UsecaseのDBクエリ回数が譜面数に依存せず、1難易度あたり2回である
- 返却件数が指定難易度の有効譜面数と一致する
- レスポンスに他難易度の行が混入しない
- `MASTER` と `ULTIMA` の両レスポンスの非圧縮サイズの合計が、既存の全レコードAPIを2人分取得する場合より小さい
...
- 同点は `DRAW`
- 自分だけプレイ済みでスコアが0より大きい場合は `SELF_WIN`
- フレンドだけプレイ済みでスコアが0より大きい場合は `FRIEND_WIN`
- 片方だけプレイ済みでもスコア0なら `DRAW`。`is_played` はレコードの有無で判定する
- 両者未プレイの場合は `DRAW`
...
- `MASTER` と `ULTIMA` の合計転送量・処理負荷を、既存の全レコードAPIを2人分取得する場合と比較する
