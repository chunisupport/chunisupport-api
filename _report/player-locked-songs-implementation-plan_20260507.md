...
このドキュメントは永続化せず、PRマージ前に削除し、適切なドキュメントへ移行します。
...
このRead Model取得は `player_locked_songs` と `songs` をJOINし、`songs.is_deleted = 0` かつ `songs.is_worldsend = 0` のレコードだけを返す。返却順は `songs.display_id ASC, player_locked_songs.is_ultima ASC` とする。JOINを使う理由は、Usecaseで `song_id` ごとに楽曲取得を繰り返すN+1問題を避けるためである。JOIN済みの `songs.title` も同時に取得し、一覧APIレスポンスへ含める。これにより、フロントエンドは未解禁曲一覧表示のためだけに楽曲マスタAPIを追加で呼び出す必要がなくなる。JOINを使わない実装にする場合でも、関連楽曲は `IN` 句などで一括取得し、1件ずつ `SongRepository` を呼び出さない。
...
一覧レスポンスには `title` を含める。未解禁曲一覧はユーザーが曲名で認識する画面であり、`display_id` だけでは表示情報として不足するためである。Read Model取得時点で既に `songs` とJOINするため、`songs.title` を追加取得するコストは小さく、フロントエンド側で楽曲マスタAPIを別途呼び出す必要もなくなる。
