...
- `DELETE /internal/me` は `users.is_deleted = true` を保存し、現在セッションのみ即時失効する
- 他セッションは一括削除されないが、次回アクセス時に削除済み判定で遅延的に無効化される
- `firebase_uid` は削除後も残るため、削除済みユーザーに紐づいた Firebase UID は再利用できない
...
- 退会後に Firebase UID を再利用できるか
- 退会を論理削除のまま運用するか
- Firebase アカウント自体の削除を本アプリが扱うか
...
退会は当面「論理削除」で継続するが、Firebase UID の再利用可否は仕様として明示する。

推奨は次のどちらかで固定する。

- 論理削除でも再登録を許すなら、退会時に `firebase_uid` を外す
- 論理削除でも再登録を許さないなら、その旨を API と UI に明示する

現状実装は後者寄りである。  
ユーザー体験を優先するなら前者に寄せる余地があるが、仕様決定が先。
...
- Firebase UID 付き削除済みユーザーの再ログイン拒否
- 再認証なし退会の拒否
- 再認証あり退会の成功
- 退会時の他セッション削除
- 退会時のリカバリーコード削除
