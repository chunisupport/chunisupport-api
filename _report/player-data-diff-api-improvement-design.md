...
```json
{
  "none": 0,
  "full_chain_gold": 0,
  "full_chain_platinum": 0
}
```

入力互換上、`fch_lv` は `1=NONE`, `2=PLATINUM`, `3=GOLD` として解釈している。集計キーは既存のフルチェインマスタ名を小文字化し、空白を `_` に置換した値に合わせる。たとえば `FULL CHAIN GOLD` は `full_chain_gold`、`FULL CHAIN PLATINUM` は `full_chain_platinum` とし、`FULL CHAIN` のプレフィックスは削除しない。
...
- フルチェイン: `none`, `full_chain_gold`, `full_chain_platinum`

## 7. 実装方針

### 7.1 計算・集計ロジックはドメイン層に置く

スコア集計、ランク・ランプ集計、レーティング計算、OVER POWER計算は、譜面成績に関するコアなビジネスルールである。そのため、具体的な計算ロジックはユースケース層ではなく、ドメインサービス・ドメインエンティティ・値オブジェクトなどのドメイン層にカプセル化する。

ユースケース層は、リポジトリから取得したドメインエンティティをドメイン層の計算器へ渡し、保存処理とレスポンス生成を組み立てるオーケストレーションのみを担当する。ハンドラはリクエスト形式の判定、DTOバインド、レスポンス返却に限定する。

インフラ層はドメインエンティティの保存・復元に責務を限定し、SQLやDBモデルへランク閾値・レーティング式・OVER POWER式などのドメイン知識を埋め込まない。
...
既存プレイヤー更新時の `before` は次の方針を比較して構築方法を決める。
...
#### 方針C: 保存後全件レコードを更新前データで差し替えて集計する

保存後に取得した通常譜面レコードの全件コピーを作成し、UPSERT前に取得済みの更新対象レコード（更新前状態）で差し替える。新規追加レコードは保存前には存在しないため、コピーから除外する。この差し替え済みデータを共通の集計器へ渡し、`before.records` と `before.profile` を集計する。

メリット:

- 保存前の全件取得を避けられる
- 方針Aと同じ集計器を再利用できるため、逆算ロジックを持たずに済む

デメリット:

- メモリ上でのレコード差し替え・除外処理の実装が必要
- 差し替えキー（譜面ID・難易度など）と新規追加判定を誤ると、`before` 集計に保存後データが混入する
...
- `combo.fc` / `combo.aj`
- `clear.hard` などのクリア件数
- `full_chain.full_chain_gold` / `full_chain.full_chain_platinum`
...
- 差分レスポンスを明示指定時のみ生成する
- 方針Cのように保存後全件レコードと更新前データを組み合わせ、保存前全件取得を避ける
- `changes` から `before` 集計を逆算する

SQL集計はランク閾値・レーティング式・OVER POWER式などのドメイン知識がインフラ層へ流出しやすいため、原則として採用しない。パフォーマンス上やむを得ずDB側の集約を検討する場合も、計算式の真実はドメイン層に置き、SQLはドメイン層で定義した仕様に従う補助的な取得手段として扱う。
