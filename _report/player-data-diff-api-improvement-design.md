...
`before.profile.overpower_percent` は、保存前レコードと同じ分母条件で計算する。これにより、`before` / `after` の差分は今回登録による変化として扱いやすくなる。
...
保存前後のレコードを入力し、`before` / `after` のプロフィール・集計値が正しく生成されることを確認する。
