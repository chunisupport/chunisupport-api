...
3. 仮定単曲レート、譜面定数、数値化公式IDの3キーでプール全体を比較する
4. 仮定状態なら本枠定員内へ入る場合だけ候補とする
...
候補条件を満たしたレコードを、仮定値ではなく現在値の3キーで並べる。
