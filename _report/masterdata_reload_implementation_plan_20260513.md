...
- 読み取り側はユースケース開始時に Provider からスナップショットを取得し、リクエスト内での一貫性を保証する。
...
5. 同時リロード要求の排他制御を実装する。
   - 仕様: 同一プロセス内での再読込は直列化（単一実行）する。
   - `ReloadMasterDataUsecase.execute` の開始点にmutexもしくはsingleflight相当の排他機構を導入する。
   - 既に再読込が進行中の場合は即時に409 Conflictを返す。
   - 共通エラーハンドラへは明示的なConflictエラーを渡す。
   - ADMIN認可フローはそのまま維持する。
...
- `go test -race ./...` （並行安全性検証のため `-race` 必須）
- `gofmt -w <変更ファイル>`
...
1. `go test -race ./...` が成功する（CI/受け入れ条件でも `-race` による並行安全性検証を実施）。
2. 再読込前後で全Usecaseが最新マスタを参照する。
3. 並行参照 + 再読込でpanicが発生しない（race detectorでも検証）。
4. 動的/静的マスタが同時に更新され、不整合が再現しない。
5. `POST /internal/master/reload` がADMIN権限でのみ実行できる。
6. 同時リロード要求が適切に排他制御され、409 Conflictが返される。
7. API仕様書が更新されている。
...
- リスク: 再読込APIの誤用で運用負荷が上がる。
  - 対策: 管理者限定とレート制限を導入する（本システムでは監査ログは導入対象外）。
